...
class MainWindow(QWidget):
    ...
    def _parse_parameters(self):
        try:
            ...
            if min_gen_int < 0 or max_gen_int < 0 or \
    ...
    def _show_results(self, table, regr):
        ...
        lin_regr = ("y = %.3f + %.3f*x1 + %.3f*x2 + %.3f*x3" % \
                               (regr[0], regr[1], regr[2], regr[3]))
        
        nonlin_regr = ("y = %.3f + %.3f*x1 + %.3f*x2 + %.3f*x3 + %.3f*x1*x2 + %.3f*x1*x3 + %.3f*x2*x3 + %.3f*x1*x2*x3" % \
    ...
    def _parse_check_parameters(self):
        try:
            ...
            if abs(gen_int) > 1 or abs(pm_int) > 1 or abs(pm_var) > 1:
            ...
            res = self.experiment.check(gen_int, pm_int, pm_var)
    ...
    def _show_table(self, res):
        ...
        lin_res = res[4]
        nonlin_res = res[5]
        res = [1, gen_int, pm_int, pm_var, \
                gen_int*pm_int, gen_int*pm_var, pm_int*pm_var, \
                gen_int*pm_int*pm_var, exp_res, lin_res, nonlin_res]
